#!/usr/bin/env python
"""Tiny forward/backward smoke test for action-UV DINO interaction tokens."""
...
def main() -> int:
    ...
    normalizer.fit(
        {
            "action": torch.randn(8, horizon, 14, device=device),
    ...
    assert grad_norm > 0.0, "Expected nonzero ActionToUVHead gradients."

    print(
        "action-uv interaction-field forward smoke test passed: "
        f"loss={loss.item():.6f}, uv_loss={loss_dict['uv_loss']:.6f}"
    )
    return 0
